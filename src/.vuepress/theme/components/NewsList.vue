<template>
  <div id="article-list" class="vp-article-list">
    ...
    <p class="sub-title" :data-item-count="String(items.length)">
      Total {{ items.length }} posts~ (｡♥‿♥｡)
    </p>
    <figure>
      <img class="news-top-img" src="https://api.isoyu.com/aipc_animation.php" alt="image"
        :key="imageKey" />
    </figure>
...
.sub-title::before {
  content: "Total " attr(data-item-count) " posts~ (｡♥‿ℂ｡)";
  position: absolute;
  color: #353434;
